...
> 알려진 제약(찾기/바꾸기·표 구조 편집 공통): 이 두 기능은 rhwp 엔진으로 문서를 다시 저장하는데, **중첩표·이미지가 많은 복잡한 문서는 엔진이 안전하게 저장하지 못해** 변경을 **거부**합니다(문서 손상 방지). 단순~중간 복잡도 문서에서 동작합니다. 복잡한 문서의 표 셀 텍스트 수정은 `propose_cell_edit`(병합 보존, XML 직접 수정)을 사용하세요.
