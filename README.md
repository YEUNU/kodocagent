...
|------|------|-----------|------|
| `.hwpx` | ✅ | ✅ | 원본 서식 보존 |
| `.hwp` | ✅ | ✅ | `.hwp` 형식 그대로 무손실 편집(`propose_edit`)·원본 자동 백업 (표·셀 구조 편집은 `.hwpx` 기반) |
...
한글 문서의 표는 병합셀(가로/세로 병합)을 흔히 사용합니다. 일반 수정 경로(`propose_edit`)는 변경된 부분만 무손실로 패치하지만, **특정 셀을 좌표로 정확히 지정**하거나 병합이 얽힌 표를 다룰 때는 `propose_cell_edit`이 더 안전합니다. `propose_cell_edit`은 `.hwpx` 내부 XML에서 **바꿀 셀의 텍스트만 직접 교체**해 병합·서식·다른 셀·양식 개체를 그대로 보존합니다.
...
문서 전체(본문·표·머리말 등)에서 텍스트를 찾아 바꿉니다("이 문서의 'OOO'를 전부 '주식회사 가나다'로 바꿔줘"). 문서 내부 XML을 직접 수정하므로 **이미지·표·서식을 보존**하며 복잡한 문서에서도 안전합니다(`.hwpx` 전용).

- 표 셀·중첩표 셀·본문 등 모든 텍스트에 적용됩니다.
- 서식이 중간에 나뉜 텍스트는 일부 누락될 수 있어, 그런 경우 알려줍니다.

### 표 행·열 편집 (구조 수정)

표에 **행·열을 추가/삭제하거나 셀을 병합**합니다("이 표 맨 아래에 행 하나 추가해줘", "1행의 1~2번째 칸 합쳐줘"). 어떤 표인지는 그 표 안의 **고유한 텍스트(anchor)**로 지정합니다(먼저 문서를 읽어 표 내용을 확인). 문서 XML을 직접 수정해 **이미지·다른 표·서식을 보존**하며, 변경 후 자동 검증으로 의도한 행·열 수가 맞는지 확인합니다(`.hwpx` 전용).

> 안전장치: 기존 병합 셀을 가로지르는 행·열 삽입/삭제는 표가 깨질 수 있어 **거부**하고 알려줍니다(병합을 먼저 해제한 뒤 시도).

### 개인정보 비식별 (마스킹)

문서의 **주민등록번호·전화번호·이메일·카드번호**를 찾아 가립니다. "확인만" 요청하면 탐지 결과만 보여주고(읽기 전용), "가려줘/마스킹/비식별"이라고 하면 마스킹 수정안을 제안합니다("이 문서 개인정보 다 가려줘"). 원문 값은 미리보기·응답 어디에도 노출하지 않으며, `.hwpx` 구조를 보존합니다.

### 문서 내보내기 (HTML / PDF)

문서를 **HTML 또는 PDF로 내보냅니다**("이 보고서 HTML로 내보내줘", "report.pdf로 저장해줘"). 원본은 그대로 두고 새 파일을 만듭니다. HTML은 항상 가능하고, PDF는 `puppeteer-core`가 설치된 환경에서만 가능합니다(미설치 시 안내).
...
긴 대화에서 컨텍스트가 무한정 커지지 않도록, 토큰 예산(`max-context-tokens`, 기본 120k)을 넘으면 **오래된 문서 읽기 결과를 자동으로 축약**합니다(최근 대화·승인 짝은 보존). 매 응답 끝과 `/context` 명령으로 **현재 컨텍스트 사용량**(사용 토큰 / 예산 / %)을, `/usage` 명령으로 **세션 누적 API 사용량**(입력·출력 토큰)을 확인할 수 있습니다.
...
> **자가 검증(v0.7.0~)**: 문서를 바꾼 뒤 에이전트가 결과를 **다시 읽어 요청이 빠짐없이 반영됐는지 스스로 점검**하고, 누락이 있으면 마저 수정합니다("모든/전부" 요청에서 일부만 처리하고 끝내는 문제를 줄임). 같은 도구를 무의미하게 반복하면 접근을 바꾸도록 자동 유도합니다.
