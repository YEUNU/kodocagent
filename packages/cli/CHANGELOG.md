...
- **API 사용량 표시를 토큰만으로** — 0.7.0의 추정 비용($) 표시를 제거하고, 사용한 누적 **입력·출력 토큰만** 표시합니다(`/usage` 및 응답·종료 요약). 모델 단가표·비용 계산 코드 제거.
- **반복 호출(thrash) 자동 억제** — 에이전트가 같은 편집 도구를 여러 번 헛호출하면, **접근을 바꾸도록(재읽기·모드 전환·한계 인정) 자동으로 안내**해 무한 반복을 막습니다. (약한 모델이 같은 도구를 18번씩 반복하던 문제 완화.)
- **`.hwp` 표·셀 편집 한계를 정직하게 안내** — `.hwp`(한/글 바이너리)의 표·셀은 kordoc이 편집을 지원하지 않습니다. 이제 변경이 적용되지 않으면 **무변경을 성공처럼 보이게 하지 않고**, "한글에서 .hwpx로 저장 후 편집 가능"하다고 명확히 알립니다(능력·한계 안내에도 반영). 실 통일부 보도자료(.hwp)로 검증 — 18회 헛호출 → 1회 시도 후 정직 안내.
