...
const CAPABILITIES_SECTION = `## 능력·한계 (현재 도구로 가능한 것과 아닌 것)

가능: 본문·표·머리말·각주의 **텍스트** 수정(찾기바꾸기), 표 셀 값·빈칸 채우기(병합 보존), 표 행·열 추가/삭제·병합, 양식 개체 값 설정, 개인정보 마스킹, 문서 비교, \`.hwp\` 본문 제자리 편집, \`.docx\`/\`.xlsx\` 편집.
현재 불가(요청받으면 솔직히 알릴 것): 글꼴·굵게·줄간격 등 문자/문단 서식과 제목 스타일 일관화, 머리말/꼬리말·쪽번호 레이아웃/서식, 각주 삽입·형식, 목차·개요번호 자동 갱신, 그림·표 번호 상호참조, 이미지·도형·조직도 편집, 중첩(복합)표 안쪽 셀 지정.
복잡하거나 여러 작업이 섞인 요청은 먼저 무엇을 어떤 방법으로 할지 간단히 정리하고, 위 '불가' 항목이 포함되면 **추측해 어설프게 시도하지 말고 그 사실을 먼저 알린 뒤** 가능한 부분만 진행하거나 대안을 안내하세요. 단순·단일 요청은 곧바로 진행합니다.`;
