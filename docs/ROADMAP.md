...
| 3 | 양식 개체 인식·채우기 정밀화 | 입력상자/누름틀 등 실제 양식 컨트롤 식별·값 설정(가능 범위 한정 명시). 실측: `extractFormFields`는 실양식에서 신뢰도 낮음(form-gibu conf 0) | 예정 — #2 후속 한계 포함: ① 빈 셀(`<hp:t/>` self-closing)은 런 미존재로 채우기 불가 ② 중첩표 안쪽 셀은 tableIndex로 지정 불가(최상위만) ③ 라벨 기반 셀 지정 미지원(현재 좌표만) |
