...
| 3 | 양식 채우기 정밀화 | 빈 칸 채우기 + 라벨 기반 셀 지정 | ◐ `e0d3d21` — ✅ 빈 셀(`<hp:t/>`) 채우기, ✅ 라벨+방향(right/below) 지정(병합 span 고려·중복 라벨 오류). 잔여: 중첩표 안쪽 셀 지정, 진짜 양식 개체(입력상자·누름틀·콤보박스) 인식(`extractFormFields`는 실양식 신뢰도 낮음 conf 0) — v1.0+ 후보 |
