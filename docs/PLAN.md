...
- v1 쓰기 범위: **HWPX(kordoc) + DOCX(docx) + XLSX(exceljs)**. (v0.6.0~) kordoc 3.x `patchHwp`로 **HWP 5.0 바이너리 직접 편집**도 지원 — `propose_edit`은 `.hwp`를 제자리 무손실 편집(표·셀 등 구조 편집 툴은 `.hwpx` 기반)
...
- `kordoc@^3.1.1` (언스코프드, TS, MIT; v0.6.0에서 `@clazic/kordoc@2.7.6`→`kordoc@3.1.1` 마이그레이션): HWP 3~5/HWPX/HWPML/PDF/DOCX/XLSX 읽기→마크다운, **무손실 패치 쓰기**(`patchHwpx`, `.hwp`는 `patchHwp`로 제자리 편집), 폼 필드 추출(`extractFormFields`), 문서 비교(`compare`). API는 throw하지 않고 결과 객체 반환
...
└── cli/         → @kodocagent/cli       # bin 엔트리, TUI, 승인 프롬프트, 온보딩, 업데이트
```
...
| `propose_edit` (hwpx 무손실 패치·hwp 제자리 편집, docx 재생성) | kordoc / docx | **필요** |
| `propose_form_fill` | kordoc `extractFormFields` + `patchHwpx` | **필요** |
...
npm 배포(`npx @kodocagent/cli@latest`/글로벌) + 24h 1회 레지스트리 체크 배너 + `kodocagent update` 셀프 업데이트. 릴리스는 GitHub Actions(OIDC 자동 발행). 상세: SPEC §9. 단일 바이너리는 보류.
...
1. ~~**HWP 5.0 바이너리 쓰기 불가**~~ **(v0.6.0 해결)**: kordoc 3.x `patchHwp`로 `.hwp`를 제자리 무손실 편집. 과거엔 kordoc·rhwp 모두 `.hwp` 쓰기 API 미노출이라 `.hwpx`로 변환 저장하는 정책이었으나, kordoc 3.1.1이 바이너리 패치 경로를 제공해 `propose_edit`에 적용(표·셀 등 구조 편집 툴은 여전히 `.hwpx` 기반)
