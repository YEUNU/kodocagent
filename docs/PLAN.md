...
> 2026-06-10 확정. 기술 검증(API 시그니처·버전·모델 ID) 반영 완료. 구현 상세는 **[SPEC.md](SPEC.md)**, 기반 조사는 [RESEARCH.md](RESEARCH.md) 참고. 변경 시 이 문서를 갱신한다.
...
| 단계 | 산출물 |
|---|---|
| **M0 스캐폴딩** | pnpm 워크스페이스, 4패키지 빌드, CI/vitest/Biome/changesets |
| **M1 루프+BYOK** | AgentSession 스트리밍·취소·JSONL 세션, 3사 프로바이더, 온보딩, `/model`, `read_document`/`list_files` |
| **M2 문서 툴** | propose 툴 전체(hwpx/docx/xlsx), 스테이징/백업/diff, 승인 UI |
| **M3 MCP+법령** | MCPManager, mcp.json 병합, korean-law 기본 번들, `LAW_OC` 온보딩 |
| **M4 폴리시+OTA** | 업데이트 체크/`update`, 한국어 에러 메시지, README, npm 첫 배포 |
| **M5 GUI (v2)** | Tauri/Electron + `@rhwp/editor` 임베드, 승인 다이얼로그 |
